...
The part I found most valuable was watching the `withLogger` HOC actually fire in the browser console — seeing `Header mounted` print (and, thanks to React's `StrictMode`, an unmount/remount right after it in development) made the mount/unmount lifecycle feel concrete in a way that reading about `useEffect` cleanup functions never quite did. The main challenge I ran into was TypeScript's `verbatimModuleSyntax` setting, which the Vite template enables by default: importing something like `PostData` or `CSSProperties` the normal way (`import { PostData } from ...`) failed, because those are types, not values, and the compiler wants type-only imports written explicitly as `import type { ... }`. I worked through it by reading the actual TypeScript error message instead of guessing, which pointed straight at the fix once I understood what "type-only import" meant. Going forward I'd like to explore state management (right now "New Post" is just a static link — I'm curious what it'd take to make it actually add a post to the list) and writing tests for components, since this project didn't cover either.
