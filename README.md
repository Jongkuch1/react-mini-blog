...
Building the Mini Blog was the first time TypeScript and React concepts I'd only understood in theory actually clicked in a real codebase. Typing the post object upfront changed how I approached the components that used it, and choosing a class component for `Post` (extending `PureComponent`) instead of defaulting to functional gave me a real reason to weigh the two options rather than just picking whichever felt familiar.

The most valuable part was watching the `withLogger` HOC actually print to the console on mount/unmount — seeing the lifecycle behave the way it's described in class, instead of just reading about it. The main challenge was TypeScript's `verbatimModuleSyntax` setting rejecting a normal import of a type (`PostData`), which needed `import type` instead — reading the actual compiler error pointed me straight to the fix. Going forward I'd like to explore state management (right now "New Post" doesn't actually add anything) and testing components, since neither came up in this project.

_(The full reflection for this project is submitted as a Canvas comment, per the assignment instructions — this section mirrors it for repo completeness.)_
